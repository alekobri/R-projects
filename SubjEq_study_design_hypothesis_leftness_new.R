...
LastRecords<-TheTable[order(TheTable$DateTime),]%>%subset(day > 4 & day != 13 & day != 20 & rel==1) %>%
...
active_feeders<-subset(Daily_mice,day > 4 & day != 13 & day != 20 & rel==1)%>%
  group_by(IdLabel, day)%>%summarise(unit1=head(unitLabel,1),unit2=tail(unitLabel,1))

table_m<-rbind(subset(active_feeders,str_detect(unit1,"l")&str_detect(unit2,"m"))[,1:2]%>%mutate(m=0),
               subset(active_feeders,str_detect(unit2,"r")&str_detect(unit1,"m"))[,1:2]%>%mutate(m=1))

LastRecords<-merge(LastRecords,table_m,all.x = T)
LastRecords<-mutate(LastRecords,leftness=ifelse(str_detect(unitLabel,"l"),1,
                                                ifelse(str_detect(unitLabel,"r"),0,m)))

###Tests####
side_preference<-LastRecords%>%group_by(m,IdLabel)%>%summarise(side_preference=mean(leftness))
t.test(side_preference$side_preference,mu = 0.5)
#l & r
t.test(subset(side_preference,is.na(m))$side_preference,mu=0.5)
#l & m
t.test(subset(side_preference,m==0)$side_preference,mu=0.5)
#r & m
t.test(subset(side_preference,m==1)$side_preference,mu=0.5)
#0-hypothesis: Mice avoid the r-feeder
t.test(subset(side_preference,m==1| is.na(m))$side_preference,mu=0.5)
...
side_preference<-mutate(side_preference,condition=as.factor(ifelse(is.na( m) , "lr",ifelse(m==0,"lm","mr"))))
stripchart(data = side_preference,side_preference~condition,vertical = T)

ggplot(side_preference, aes(condition, side_preference, col = IdLabel, geom="point")) +
